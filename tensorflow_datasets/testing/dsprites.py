...
def _create_fake_samples():
  ...
  return images, np.transpose(classes), np.transpose(values)
...
def _generate():
  ...
  with h5py.File(os.path.join(output_dir, OUTPUT_NAME), "w") as f:
    ...
    calues_dataset = f.create_dataset("latents/values", values.shape, "<f8")
    calues_dataset.write_direct(np.ascontiguousarray(values))
